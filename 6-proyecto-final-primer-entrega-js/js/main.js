const claseJavascript = "Curso de Javascript - Primer Entrega Proyecto - Clase 6";
...
function miCarrito() {
    ...
    let productoSeleccionado = parseInt((prompt("Ingreá el Id del producto que querés comprar:")) - 1);
    ...
    if ((objIndex + 1) != 0) {
        ...
        //Muestro y actualizo stock de productos
        recorreArrayProdcutos(arrayProductos);

        //Muestro y actualizo carrito
        recorreArrayCarrito(carritoArray);

        //Si finaliza comprar pido datos necesarios
        let finalizarCompra = prompt("SI QUERÉS FINALIZAR TU COMPRA ESCRIBÍ: Si").toLowerCase();
        if (finalizarCompra == "si") {
            ...
            alert("Pido la cantidad de datos que quiera");

            // Muestro mensaje de finalizar compra
            document.getElementById("mensaje").innerHTML = "Gracias <b>" + nombre + "</b> por tu compra.<br>Tu pedido tiene el N° <b>XXXX</b><br>Te vamos a contactar para coordinar la entrega.<br>Saludos Mi Tienda."
        }
        //Si NO finaliza vuelvo a mostrar el carrito
        else {
            miCarrito()
        }
    }
